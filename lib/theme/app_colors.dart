import 'package:flutter/material.dart';

class AppColors {
  AppColors._();

  // 主色
  static const Color primary = Color(0xFFFFC857); // 明亮活力黄橙
  static const Color secondary = Color(0xFF4A90E2); // 辅助蓝
  static const Color surface = Color(0xFFFFFFFF); // 卡片/输入框背景
  static const Color textPrimary = Color(0xFF222222); // 主文本
  static const Color textSecondary = Color(0xFF888888); // 次文本
  static const Color border = Color(0xFFDDDDDD); // 分割线/边框
  static const Color success = Color(0xFFB6E388); // 成功/绿色按钮
  static const Color warning = Color(0xFFFFE082); // 警告
  static const Color error = Color(0xFFFF6F61); // 错误
  static const Color icon = Color(0xFF222222); // 图标
}